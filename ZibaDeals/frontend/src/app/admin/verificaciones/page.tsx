...
export default function AdminVerificationsPage() {
    ...
    const handleReject = async (verification: Verification) => {
        ...
        await Promise.all([
            supabase
                .from('verifications')
                .update({ status: 'rechazado', reviewed_at: now, rejection_reason: rejectionReason })
                .eq('id', verification.id),
            supabase
                .from('profiles')
                .update({
            ...
            supabase.from('audit_logs').insert({
                ...
                target_table: 'verifications',
                target_id: verification.id,
                new_data: { status: 'rechazado', reason: rejectionReason },
            }),
        ])
            ...
            <header className="bg-white border-b border-[var(--madui-border)] py-4">
                <div className="max-w-5xl mx-auto px-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-lg font-bold text-[var(--madui-text)]">
                                Panel Admin
                            </h1>
                ...
                <section>
                    ...
                    {pending.length === 0 ? (
                        <div className="text-center py-8 bg-white rounded-xl border border-[var(--madui-border)]">
                            ...
                            <p className="text-sm text-[var(--madui-text-muted)]">
                        ...
                        <div className="space-y-4">
                            {pending.map((v) => (
                                <div
                                    key={v.id}
                                    className="bg-white rounded-xl border border-[var(--madui-border)] overflow-hidden"
                                >
                                    {/* Summary row */}
                                    <div className="p-4 flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-xs text-[var(--madui-text-muted)]">
                                                {v.profiles?.business_name} · {v.profiles?.business_type} · Enviado{' '}
                                                {new Date(v.submitted_at).toLocaleDateString('es-MX')}
                                            </p>
                                        </div>
                                        <button
                                            onClick={() => setSelectedId(selectedId === v.id ? null : v.id)}
                                            className="text-sm font-medium text-[var(--madui-primary)] hover:underline"
                                        >
                                    ...
                                    {selectedId === v.id && (
                                        <div className="border-t border-[var(--madui-border)] p-4 bg-gray-50">
                                            {/* Documents */}
                                            <p className="text-sm font-semibold text-[var(--madui-text)] mb-3">
                                                Documentos
                                            </p>
                                                ...
                                                {[
                                                    ...
                                                    <div key={doc.label} className="text-center">
                                                        <a
                                                            ...
                                                            className="block p-3 bg-white rounded-lg border border-[var(--madui-border)] hover:border-[var(--madui-primary)] transition-colors"
                                                        >
                                                            <span className="text-2xl block mb-1">📄</span>
                                                            <span className="text-xs font-medium text-[var(--madui-primary)]">
                                                                {doc.label}
                                                            </span>
                                                        </a>
                                                    </div>
                                            ...
                                            <div className="flex flex-col sm:flex-row gap-3">
                                                <button
                                                    ...
                                                    disabled={processing}
                                                    className="px-5 py-2.5 bg-green-600 text-white text-sm font-semibold rounded-xl hover:bg-green-700 transition-colors disabled:opacity-50"
                                                >
                                                    ✓ Aprobar
                                                </button>
                                                <div className="flex-1 flex gap-2">
                                                    <input
                                                        ...
                                                        className="flex-1 px-4 py-2.5 border border-[var(--madui-border)] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-300 focus:border-red-400"
                                                    />
                                                    <button
                            ...
                            {processed.slice(0, 20).map((v) => (
                                <div key={v.id} className="p-4 flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-xs text-[var(--madui-text-muted)]">
                                            {v.profiles?.business_name}
                                        </p>
                                    </div>
                                    <span
                                        className={`text-xs font-semibold px-3 py-1 rounded-full ${v.status === 'aprobado'
